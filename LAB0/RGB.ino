/*
  ...
  Modified for Adafruit HUZZAH32 ESP32 Feather (4/24/2019)
 */
 
// Set up the rgb led names
uint8_t ledR = 21; // GPIO21 for red   LED
uint8_t ledG = 17; // GPIO17 for green LED
uint8_t ledB = 16; // GPIO16 for blue  LED
...
uint8_t brightness = 255;  // 255 is maximum brightness, but can be changed.
                           // (Might need 256 for a common anode RGB LED to fully turn off).

// the setup() routine runs just once when you press reset:
void setup() 
{            
  Serial.begin(115200);   // Open serial communications line to host PC
  delay(10);              // Pauses program execution for 10 milliseconds
...
void loop() 
{
  Serial.println("Show sequence of primary LED colors (R, G, B).");
  ...
  ledcWrite(1, 255);  ledcWrite(2,   0);  ledcWrite(3,   0); // Red only
  delay(3000);
  ledcWrite(1,   0);  ledcWrite(2, 255); // Blue only
  delay(3000);
  ledcWrite(2,   0);  ledcWrite(3, 255); // Green only
  delay(3000);
